...
If you hit, you affect them with a Contact poison using your normal Toxicology Skill Ranks or (3 + XP/200), whichever is higher, to determine its Potency and Duration, which is modified further by the types of Snakes you utilize:

 - **Black Adder** *(Chakra +5)* - Your Toxicology roll with Sen'eijashu has a +3 bonus for each Black Adder summoned, and it's Poison Severity is increased by 1.
 - **Black Mamba** *(Chakra +5)* - This technique begins with Suffocation 0. Every 25 IC, the Suffocation status increases by (Sen'eijashu's Poison Severity/2).
 - **Blue Krait** *(Chakra +10)* -  Your victim suffers an Immobilize with a severity equal to Sen'eijashu's Poison Severity.
 - **King Cobra** *(Chakra +10)* - For the duration of the poison, the victim has a -4 penalty to Stamina and Chakra Exhaustion rolls, and a -4 penalty to all their non-Resistance skill rolls, along with Paralysis 3.
 - **Pit Viper** *(Chakra +10)* - Bleed 3. For the poison's duration, any Bleed effects on the victim will not decrease naturally.
 - **Puff Adder** *(Chakra +10)* - The puff adder's venom has the unusual, and fairly terrifying, effect of causing widespread cellular death near the site of the bite. If the victim is poisoned, roll for location (unless a wound was inflicted using Sen'eijashu, in which case use that location), for a Minor Piercing wound. 50 IC after being poisoned, the victim suffers a Major wound on that location. 100 IC later, they suffer a Severe wound.
 ...
 - **Taipan** *(Chakra +5)* - Sen'eijashu has a +2 Accuracy bonus. You may make Sen'eijashu a Called Shot, with no Accuracy penalty, and its Duration is doubled.
...
**Emperor Cobra Venom** *(Injury, Ingestion, Contact, Inhalation)*  
 ...
 - Paralysis: 4
 - Duration: 75
 - Toxicology Ranks Required: 25 (to be used by a human)

In the case of someone with Rare Poisons, you may utilize Emperor Cobra Venom as one of its two 'special effects', which grants all of its statistics, overriding their normal values if they are better (i.e., if your Poison would normally be Severity 3, it becomes Severity 4; if it would have a higher Severity or longer Duration, however, use the better of the two).  The actual Special Effect (if, say, someone uses an antidote) of Emperor Cobra Venom, however, is Paralysis 4.
