...
You grab onto an opponent, with the intent to not let go! It's not a preferred method of fighting among ninja, but it does have some limited popularity amongst certain taijutsu fighters.
 ...
 - Stamina: 6 if it misses, 12 if it hits
 - Speed: 12
...
Certain situations within a grapple may call for a 'grapple check'. This varies depending on whether you are the attacker or defender: The Attacker rolls 1d20 + PDB + AGI/15, while the Defender rolls 1d20 + PDB + DEX/15, with both people rounding down their Damage Bonus to the nearest whole number (7.2 and a 7.9 both round down to 7).
...
Effects: When you perform Lock, choose one of the three basic body parts of your opponent to lock: Head, Arm, or Leg.  You are required to use an arm to lock an opponent's head or arm (meaning you could do one of both, or two arm locks, but not a headlock and double arm lock), and a leg for a leg lock.

 - Head: Immobility 4, -2 to Grapple rolls, and a -4 Stamina penalty.  Any jutsu or effects which require them to aim with their mouth (such as breathing fire or spitting out needles) have a -5 Accuracy penalty. 
 - Arm: Immobility 2, -3 to Grapple rolls.  With one arm locked, Seal Speeds are doubled (if currently forming seals, the remainder is doubled), and they can not wield weapons two-handed.  With both arms locked, your opponent can not form handseals at all, and can only utilize Small weapons. A person with the Single-Handed Seals unique is able to perform handseals one-handed unhindered by these effects.
 - Leg: Immobiliy 3, -2 to Grapple rolls.  With one leg locked, you further halve the speed at which you and they can move in the grapple.  With both legs locked, neither you nor they can move at all (reducing your movement speeds to 0).

 The Immobility and Grapple roll penalties from applying two Arm or Leg Locks are doubled likewise, but Immobility penalties from different locks do not add together, and you apply simply the highest penalty amongst them.  Grapple roll penalties from different locks, however, all stack with one another. 
 
All penalties from Locks remain (and will not reduce over time as normal for Immobility penalties, or clear on Fatiguing for a Head Lock's Stamina penalty) until you lose contro lof or end the grapple.
...
Effects: You lose the ability to use one of your arms while maintaining this hold. Every use of this technique applies Suffocation 1 to your opponent. So long as this hold is maintained (meaning you don't use the arm holding them for anything else, and remain in control of the grapple) the Suffocation status does not reduce.
...
You grab your opponent in such a way that you can put continual strain on one of their limbs or joints. Though typically non-lethal, an expert grappler can be brutal with these techniques.
 ...
 - Speed: 8
 - Requires: Must have your target in an Arm Lock or Leg Lock.

Effects: If you have multiple limbs locked, declare which one you are targeting when you use this technique: Any wounds dealt by it are applied to that limb. Although the damage is dealt normally, it is considered cumulatively 15% higher each time you use the technique after the first, so long as the lock is maintained. For example, the first Joint Lock you apply does unarmed damage; the next, unarmed damage +15%; the third +30%, and so forth, and so on. If you lose the lock, or the grapple ends, then the cumulative bonus is reset. You can apply Power Attack when using Joint Lock, increasing its damage and Stamina cost accordingly, as well as Unarmed Focus.
...
A devastating move, developed by a shinobi who claimed he had gained inspiration for it by watching the fighting tactics of wild bears. The user wraps their arms around their opponent and squeezes with incredible strength. Users of this move have been known to crack arm bones and break ribs. It was later pointed out that real bears do not actually hug people, to death or otherwise. The original inventor of the move refused to comment, and the name was in wide use by that point anyway.
 ...
 - Damage: (4 + STR/6)d12
 - Speed: 16
 - Requires: Your target to have both arms locked and with Compression Hold applied once to one of the locks

Effects: Your Physical Damage Bonus is doubled for this technique. If this technique causes a wound, roll 1d3; on a 1, the wound is dealt to the opponent's Torso; on a 2 or 3, it is dealt to an arm (left or right, respectively). Using KuroKumaKen ends ends any existing Arm Lock you have on the target.
...
Effects: This may be used outside of a grapple; in fact, it may only be used outside of a grapple. If you hit, you deal damage and initiate a grapple with your opponent, and may immediately apply any one Lock or choose to knock them prone automatically. Additionally, make an opposed Athletics roll: If you beat your target's roll, they must immediately make a Stamina 5 + X roll, with X being how much you beat their Athletics roll by, to a maximum of Stamina 25.
...
Effects: You must use both your arms to keep your opponent in this hold. Apply the penalties of a Head lock and a double arm lock, except for the Immobility and Grapple roll penalties, which are instead -8 from this technique (instead of the lock's normal penalties).  You can utilize Joint Lock on your target's arms and/or Choke Hold while maintaining Full Nelson. This ends if you lose control of the grapple; notably, this does not count as having an opponent in a Double Arm Lock for, say, KuroKumaKen, even though it applies those penalties.
...
Effects: Shishi Keishi is performed from outside of a grapple. If this technique misses, the "X" value is not paid. When performing Shishi Keishi, declare any grapple move of B rank or below which can be used during a grapple, except for KuroKumaKen; if it hits, you initiate a grapple and then immediately perform that move. "X" is the cost and speed of the move you are using with Shishi Keishi.
